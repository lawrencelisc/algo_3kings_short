...
def scouting_weak_coins(n=20):
    ...
    try:
        ...
        # 於 Top 20 大幣中，選出跌幅最大 (最弱勢) 的 n 隻幣
...
def manage_short_positions():
    ...
    try:
        ...
        for s in list(positions.keys()):
            ...
            # 🚀 階段一 & 二：跌幅超過 1.5 倍 ATR 才推保本 (防空軍被震倉)
            if not pos['is_breakeven'] and pnl_pct > (coin_volatility_pct * 1.5):
                # 做空保本位：入場價跌 0.2% (確保夠俾手續費)
                pos['sl_price'], pos['is_breakeven'], sl_updated = pos['entry_price'] * 0.998, True, True

            # 🚀 階段三：三段式放風箏追蹤止損 (Trail SL - 做空版)
            if pos['is_breakeven']:
                # 暴跌期：利潤飆升超過 3 倍 ATR，風箏線立刻收緊至 1.0 ATR 距離，準備食糊
                if pnl_pct > (coin_volatility_pct * 3.0):
                    trail_sl = curr_p + (1.0 * pos['atr'])  # 做空止損在現價之上 (+)
                # 正常下跌期：風箏線放鬆至 2.0 ATR 距離，容許合理反彈
                else:
