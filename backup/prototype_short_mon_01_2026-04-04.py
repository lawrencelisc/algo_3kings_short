...
import json  # ✅ 新增：用於 Dashboard 通訊
...
# ==========================================
# 101 新增功能：Dashboard 狀態更新 Function
# ==========================================
def update_bot_status(msg):
    """將即時訊息寫入 JSON，讓 Dashboard 可以同步顯示"""
    try:
        current_balance = get_live_usdt_balance()
        status_payload = {
            "last_update": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "status_msg": msg,
            "balance": round(current_balance, 2),
            "active_positions": list(positions.keys()),
            "cooldown_count": len(cooldown_tracker)
        }
...
def apply_lee_ready_short_logic(symbol):
    ...
    try:
        ...
        if is_weak:
            # ❌ 原代碼
            # print(f"📉 {symbol} Short Validated | Z-Score: {z_score:.2f} | Imbalance: {imbalance:.2f} | P95 Cap: {p95:.0f}")
            # ✅ 新代碼
            msg = f"📉 {symbol} Short Validated | Z-Score: {z_score:.2f} | Imbalance: {imbalance:.2f} | P95 Cap: {p95:.0f}"
            print(msg)
            update_bot_status(msg)
        elif z_score < -NET_FLOW_SIGMA:
            # ❌ 原代碼
            # print(f"⚠️ {symbol} Fake-Drop Prevented | Z-Score: {z_score:.2f} but Imbalance is {imbalance:.2f} (Buy Wall in the way)")
            # ✅ 新代碼
            msg = f"⚠️ {symbol} Fake-Drop Prevented | Z-Score: {z_score:.2f} but Imbalance is {imbalance:.2f} (Buy Wall in the way)"
            print(msg)
            update_bot_status(msg)
    ...
    try:
        ...
        for s in list(positions.keys()):
            ...
            if exit_reason:
                ...
                # ✅ 新代碼
                msg = f"⚔️ 觸發 {exit_reason}，執行 IOC 平空單: {s}"
                print(msg)
                update_bot_status(msg)
                ...
                if ioc_pnl > 0:
                    ...
                    # ✅ 新代碼
                    msg = f"🏆 {s} Bot 主動止盈平倉！解除冷卻，允許乘勝追擊！"
                    print(msg)
                    update_bot_status(msg)
                    if s in cooldown_tracker:
                        del cooldown_tracker[s]
...
def execute_live_short(symbol, net_flow, current_price, is_weak, atr, is_volatile):
    ...
    try:
        ...
        if actual_amount == 0:
            ...
            # ✅ 新代碼
            msg = f"⏩ {symbol} IOC 未成交或數量為 0，執行核彈撤單並退出。"
            print(msg)
            update_bot_status(msg)
            cancel_all_v5(symbol)
            return
        ...
        try:
            ...
            # ✅ 新代碼
            msg = f"✅ {symbol} 止盈止損已設置 | TP: {tp_p} | SL: {sl_p}"
            print(msg)
            update_bot_status(msg)
        except Exception as e:
        ...
        # ✅ 新代碼
        msg = f"📉 [已入貨做空] {symbol} @ {actual_price:.4f} | 數量: {actual_amount}"
        print(msg)
        update_bot_status(msg)
...
# ==========================================
def main():
    msg1 = f"🚀 AI 實戰 V6.1 FINAL SHORT 啟動..."
    msg2 = f"Lee-Ready 資金流邏輯 + 訂單簿失衡度 (Imbalance) + P95濾網 [終極做空版] + 監控 啟動..."
    print(msg1)
    print(msg2)
    update_bot_status(msg1 + " | " + msg2)
    ...
    while True:
        try:
            ...
            if curr_t - last_scout_time > SCOUTING_INTERVAL:
                ...
                # ✅ 新代碼
                msg = f"⏳ 空軍巡邏完畢 | 持倉: {list(positions.keys())} | 餘額: {get_live_usdt_balance():.2f}"
                print(msg)
                update_bot_status(msg)
